...
async function scrapePage(pageUrl) {
  console.log(visitedUrls.size, "SIZE");
  ...
  const pageTextArray = extractTextData(html);
  pageTextArray.forEach((text) => textSet.add(text));
...
async function scrapeWebsite(baseUrl) {
  ...
  // const allTextData = Array.from(textSet).join(" ").replace(/\s+/g, " ");
  const allTextData = Array.from(textSet).join(" ").replace(/\s+/g, " ");
  const output = { text: allTextData };
...
//       const loader = new CheerioWebBaseLoader(link, options);
